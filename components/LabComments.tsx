...
export default function LabComments({ pageId }: { pageId?: string }) {
  ...
  useEffect(() => {
    ...
    // 安全保护：未配置 gitalkConfig 或 clientID 为空时不渲染
    const gk: any = (siteConfig as any).gitalkConfig;
    if (!gk || !gk.clientID || !gk.clientSecret || !gk.repo || !gk.owner) {
      containerRef.current.innerHTML = '<div style="padding:20px;text-align:center;color:#94a3b8;font-size:14px;">💬 留言板暂未配置（缺少 GitHub OAuth 凭证）</div>';
      return;
    }
    ...
    const gitalk = new Gitalk({
      clientID: gk.clientID,
      clientSecret: gk.clientSecret,
      repo: gk.repo,
      owner: gk.owner,
      admin: gk.admin || [gk.owner],
      proxy: '/api/github',
